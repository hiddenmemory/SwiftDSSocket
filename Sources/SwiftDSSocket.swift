...
@objc public protocol SwiftDSSocketDelegate {
    ...
    @objc optional func socket(sock: SwiftDSSocket, didAcceptNewSocket newSocket: SwiftDSSocket, hostname: String, port: UInt16)
}
...
public class SwiftDSSocket: NSObject {
    ...
    public func re(delegate: SwiftDSSocketDelegate?, queue: DispatchQueue?) {
        self.delegate = delegate
    ...
    @inline(__always)
    static func local_assert(_ expr: Bool, _ errorMessage: String, _ block: () -> Void = {}) {
        if !expr {
            NSLog("%@", errorMessage)
            block()
            exit(1)
    ...
    fileprivate func doAccept(_ socketFD: Int32) {
        ...
        delegateQueue?.async {
            self.delegate?.socket?(sock: self, didAcceptNewSocket: childSocket, hostname: peerHostName, port: peerPort)
        }
    }
